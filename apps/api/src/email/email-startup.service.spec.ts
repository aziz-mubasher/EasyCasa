...
const base = (over: Partial<ApiConfig>): ApiConfig =>
  ({ ALLOW_PROVIDER_STUBS: false, SMTP_URL: '', EMAIL_PROVIDER_URL: '', ...over }) as ApiConfig;

describe('EmailStartupService', () => {
  it('logs ERROR when production posture and email transport unset', () => {
    const errorSpy = vi.spyOn(Logger.prototype, 'error').mockImplementation(() => undefined);
    const svc = new EmailStartupService(base({ ALLOW_PROVIDER_STUBS: false }));
    svc.onModuleInit();
  ...
  it('is silent when ALLOW_PROVIDER_STUBS=true EC_TEST_AUTH=true', () => {
    const errorSpy = vi.spyOn(Logger.prototype, 'error').mockImplementation(() => undefined);
    const svc = new EmailStartupService(base({ ALLOW_PROVIDER_STUBS: true }));
    svc.onModuleInit();
