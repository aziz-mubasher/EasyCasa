...
| VALUATION_BAND_ENABLED | api | Serve `GET /listings/:slug/valuation-band` and `POST /avm/band` (stub comparables provider). Default `false` until enabled per environment. |
| SHARE_VIEW_PEPPER | api | HMAC secret for SmartLink visitor hashes (`share_link_visitor_hashes`). **Required in production**; empty falls back to a dev default. Never stores raw IP. |
