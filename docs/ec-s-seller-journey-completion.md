...
**Suggested dispatch order:** PP-4 → PP-5 → PP-6 → PP-1 → PP-2 (+PP-3 folded into any of them). After AZM decision: V-1 flip (ops) anytime independent of eng order.
...
| PK-3 | Analytics flip | Stage 7 live → P7 ledger deepening |
...
## 3. Journey definition of done (smoke, run after PP-4 + PP-5 + V-1 flip)
...
4. Availability set → buyer books → seller confirms → completes (requires V-1 flip).
