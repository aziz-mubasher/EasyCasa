...
**K EC 7.4 / EC-21 (channel + API Hub):** same module — first-contact language → welcome/off-hours → `book_viewing` / `search_brief` / `open_listings`; `wa_contacts` + CRM `source=whatsapp`; Hub tabs `#whatsapp/{connection,templates,canned,analytics}`. No Assist/Consult, no B4A credit copy, own WABA. See `docs/whatsapp-channel.md`.
